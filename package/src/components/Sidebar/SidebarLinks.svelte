...
<div class="flex grow flex-col gap-y-5 overflow-y-auto bg-sky-600 px-6 pb-4">
  <div class="flex h-16 shrink-0 items-center">
    <img class="h-8 w-auto" src="https://tailwindui.com/img/logos/mark.svg?color=white" alt="Your Company">
  </div>

  <nav class="flex flex-1 flex-col">
    <div class="text-xs font-semibold leading-6 text-sky-200">Your collections</div>
      ...
      <li>
        <ul role="list" class="-mx-2 space-y-1">
          {#each Object.keys($page.data.collections) as collection}
            ...
            <li>
              <a href="/leblog/collections/{collection}" class="{ is_active ? 'bg-sky-700 text-white' : 'text-sky-200 hover:text-white hover:bg-sky-700' } group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold">
                {collection}
              </a>
            </li>
          {/each}
